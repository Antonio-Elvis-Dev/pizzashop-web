...
test('paginate orders', async ({page})=>{
    ...
    expect(page.getByRole('button', { name: 'Próxima página' }).click())
    ...
    expect(page.getByRole('button', { name: 'Última página' }).click())
    ...
    expect(page.getByRole('button', { name: 'Primeira página' }).click())
    expect(page.getByRole('cell', { name: 'Customer 1', exact: true })).toBeVisible()
    expect(page.getByRole('cell', { name: 'Customer 10', exact: true })).toBeVisible()
    await page.waitForTimeout(1000)
...
test('filter by order id', async({page})=>{
    ...
    expect(page.getByRole('cell',{name:'order-4', exact:true})).toBeVisible()
